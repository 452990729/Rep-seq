...
class Fastq(object):
    ...
    def CutP(self, length, start):
        return '\n'.join([self.f, self.seq[:start]+self.seq[start+length:],\
                         self.t, self.q[:start]+self.q[start+length:]])
...
def HnadleFq(file_in, dict_fa, list_len, start, mismatch):
    m = 0
    list_tmp = []
    dict_tmp = {}
    base = re.split('_', os.path.basename(file_in))[0]
    out = open(base+'_CutPrime.fastq', 'w+')
...
def main():
    ...
    HnadleFq(sys.argv[2], dict_fa, list_len, int(sys.argv[3]), 2)
