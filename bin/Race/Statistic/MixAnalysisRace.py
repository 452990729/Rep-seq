...
def ValueTest(path_in, tp):
    ...
    out_v = CalP(np_v, group_len, tp)
    out_vj = CalP(np_vj, group_len, tp)
    out_vdj = CalP(np_vdj, group_len, tp)
    out_clone = CalP(np_clone, group_len, tp)
    f = xlwt.Workbook()
    ...
    f.save(os.path.join(path_base, 'MixDiffAnalysisBy{}test.xls'.format(tp)))
...
def main():
    if len(sys.argv) == 2:
        ValueTest(sys.argv[1], 'T')
        ValueTest(sys.argv[1], 'KS')
    elif len(sys.argv) == 3:
        ValueTest(sys.argv[1], sys.argv[2])
