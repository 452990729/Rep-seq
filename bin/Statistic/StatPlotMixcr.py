...
def PlotCDR3Bar(list_path, tp, pathout):
    ...
    if len(list_path) == 2:
        ...
        assert len(list_path)%2 == 0
        fig, axes = plt.subplots(nrows=len(list_path)/2, ncols=2)
        m = 0
        n = 0
        c = 0
        for path in sorted(paths):
...
def PlotVJ(path_in, xlb):
    ...
    axs[-1].axis('off')
    fig.subplots_adjust(wspace=1)
    fig.suptitle('Usage of VJ genes ({})'.format(xlb))
    plt.savefig(os.path.join(dir_s, 'FractionOfVJOf{}.png'.format(xlb)), bbox_inches='tight')
...
def main():
    ...
    elif sys.argv[1] == 'PlotVJ':
        PlotVJ(sys.argv[2], sys.argv[3])
